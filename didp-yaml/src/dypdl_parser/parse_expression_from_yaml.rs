...
mod tests {
    ...
    fn parse_integer_from_yaml_ok() {
        ...
        let result = parse_integer_from_yaml(&value, &metadata, &registry, &parameters);
        assert!(result.is_ok());
        assert_eq!(
            result.unwrap(),
    ...
    fn parse_continuous_from_yaml_ok() {
        ...
        let value = Yaml::String(String::from("(+ cost 1)"));
        let result = parse_continuous_from_yaml(&value, &metadata, &registry, &parameters);
        assert!(result.is_ok());
        assert_eq!(
            result.unwrap(),
    ...
    fn parse_set_from_yaml_ok() {
        ...
        let result = parse_set_from_yaml(&value, &metadata, &registry, &parameters);
        assert!(result.is_ok());
    ...
    fn parse_vector_from_yaml_ok() {
        ...
        let result = parse_vector_from_yaml(&value, &metadata, &registry, &parameters);
        assert!(result.is_ok());
        assert_eq!(
            result.unwrap(),
