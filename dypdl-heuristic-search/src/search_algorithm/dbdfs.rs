...
///
/// let mut solver = Dbdfs::<_, FNode<_>, _>::new(input, transition_evaluator, parameters);
/// let solution = solver.search().unwrap();
/// assert_eq!(solution.cost, Some(1));
/// assert_eq!(solution.transitions, vec![increment]);
/// assert!(!solution.is_infeasible);
/// ```
pub struct Dbdfs<'a, T, N, E, V = Transition>
where
...
impl<'a, T, N, E, V> Dbdfs<'a, T, N, E, V>
where
    T: variable_type::Numeric + Ord + fmt::Display,
    N: BfsNode<T, V>,
    E: Fn(&N, Rc<V>, &mut StateRegistry<T, N>, Option<T>) -> Option<(Rc<N>, bool)>,
...
impl<'a, T, N, E, V> Search<T> for Dbdfs<'a, T, N, E, V>
where
    T: variable_type::Numeric + Ord + fmt::Display,
    N: BfsNode<T, V>,
    E: Fn(&N, Rc<V>, &mut StateRegistry<T, N>, Option<T>) -> Option<(Rc<N>, bool)>,
