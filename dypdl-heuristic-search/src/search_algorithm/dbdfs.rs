...
/// ```
/// use dypdl::prelude::*;
/// use dypdl_heuristic_search::search_algorithm::{Dbdfs, Search};
...
/// };
///
/// let mut solver = Dbdfs::<_, FNode<_>, _, _>::new(model, h_evaluator, f_evaluator, true, 1, parameters);
/// let solution = solver.search().unwrap();
/// assert_eq!(solution.cost, Some(1));
/// assert_eq!(solution.transitions, vec![increment]);
/// assert!(!solution.is_infeasible);
/// ```
pub struct Dbdfs<T, N, H, F>
where
...
impl<T, N, H, F> Search<T> for Dbdfs<T, N, H, F>
where
    T: variable_type::Numeric + Ord + fmt::Display,
    N: BfsNodeInterface<T>,
    H: Fn(&StateInRegistry) -> Option<T>,
    ...
    fn search_next(&mut self) -> Result<(Solution<T>, bool), Box<dyn Error>> {
        while !self.open.is_empty() || !self.next_open.is_empty() {
            if self.open.is_empty() {
                ...
                if !self.quiet {
                    println!("New discrepancy limit: {}", self.discrepancy_limit);
                }
            }
