...
#[derive(Debug, Clone)]
pub struct FNode<T, V = Transition>
where
    T: Numeric,
    ...
    Transition: From<V>,
{
        ...
        F: FnOnce(T, T, &StateInRegistry) -> T,
    {
        ...
        Some(FNode::new(state, cost, h, f, None, None))
    }
        ...
        F: FnOnce(T, T, &StateInRegistry) -> T,
    {
        ...
        Some(FNode::new(state, g, h, f, Some(self), Some(transition)))
    }
        ...
        M: Deref<Target = Model> + Clone,
    {
        ...
        let constructor = |state, g, other: Option<&FNode<T, V>>| {
...
impl<T, V> PartialEq for FNode<T, V>
where
...
impl<T, V> PartialOrd for FNode<T, V>
where
    T: Numeric + Ord,
    V: TransitionInterface + Clone,
    Transition: From<V>,
...
impl<T, V> StateInformation<T, Rc<HashableSignatureVariables>> for FNode<T, V>
where
    T: Numeric,
    V: TransitionInterface + Clone,
    Transition: From<V>,
...
impl<T, V> GetTransitions<V> for FNode<T, V>
where
    T: Numeric + Ord,
    V: TransitionInterface + Clone,
    Transition: From<V>,
...
impl<T, V> BfsNode<T, V> for FNode<T, V>
where
...
mod tests {
    ...
    fn cmp_min() {
        ...
        let f_evaluator = |g, h, _: &StateInRegistry| g + h;
        let node1 = FNode::<_>::generate_root_node(
            state.clone(),
            0,
            &model,
            &h_evaluator_0,
        ...
        let node4 =
            FNode::<_>::generate_root_node(state, 0, &model, &h_evaluator_1, &f_evaluator, None);
