...
/// model.add_forward_transition(transition).unwrap();
///
/// let constraints = TransitionConstraints::new(&model, false);
///
/// assert_eq!(constraints.get_forbidden_before(false, 0), &[]);
/// assert_eq!(constraints.get_forbidden_before(true, 0), &[]);
/// assert_eq!(constraints.get_forbidden_before(true, 1), &[(false, 0)]);
/// assert_eq!(constraints.get_forbidden_before(false, 1), &[(true, 0)]);
///
/// assert_eq!(constraints.get_forbidden_after(false, 0), &[(true, 1)]);
/// assert_eq!(constraints.get_forbidden_after(true, 0), &[(false, 1)]);
/// assert_eq!(constraints.get_forbidden_after(true, 1), &[]);
/// assert_eq!(constraints.get_forbidden_after(false, 1), &[]);
///
/// let model = Rc::new(model);
/// let generator = SuccessorGenerator::<_, Rc<_>>::from_model_without_custom_cost(model, false);
...
/// let generator = constraints.filter_successor_generator(&generator, prefix, suffix);
...
impl TransitionConstraints {
    ...
    pub fn filter_successor_generator<U, R>(
        &self,
        generator: &SuccessorGenerator<TransitionWithCustomCost, U, R>,
        prefix: &[TransitionWithCustomCost],
        suffix: &[TransitionWithCustomCost],
    ...
    /// Create a new transition constraints object.
...
mod tests {
    ...
    fn get_forbidden_before_with_no_backward_transitions() {
        ...
        let constraints = TransitionConstraints::new(&model, true);
        constraints.get_forbidden_before(false, 0);
    }
    ...
    fn filter_successor_generator_without_any_constraints_and_suffix() {
        ...
        let constraints = TransitionConstraints::new(&model, false);
        let expected = SuccessorGenerator::<_>::from_model_without_custom_cost(model, false);
    ...
    fn filter_successor_generator_with_constraints_and_prefix() {
        ...
        let constraints = TransitionConstraints::new(&model, false);
        let generator =
            SuccessorGenerator::<_>::from_model_without_custom_cost(model.clone(), false);
        let t1 = TransitionWithCustomCost {
            id: 0,
            forced: false,
            transition: t1,
            ..Default::default()
        };
        let t2 = TransitionWithCustomCost {
            id: 0,
            forced: true,
            transition: t2,
            ..Default::default()
