...
use crate::greedy_lookahead_bfs;
use crate::solver;
...
/// Best-first search with bounded depth-first lookahead solver using the dual bound.
///
...
pub struct DualBoundGreedyLookaheadBFS<T: variable_type::Numeric> {
...
impl<T> solver::Solver<T> for DualBoundGreedyLookaheadBFS<T>
where
    ...
    fn solve(&mut self, model: &dypdl::Model) -> Result<solver::Solution<T>, Box<dyn Error>> {
        ...
        let solution = match self.f_evaluator_type {
            FEvaluatorType::Plus => {
                ...
                greedy_lookahead_bfs::greedy_lookahead_bfs(
                    model,
                    generator,
                    &h_evaluator,
                    f_evaluator,
