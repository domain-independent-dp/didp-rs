...
pub struct SendableSuccessorIterator<'a, T, N, V>
where
    T: Numeric + Display,
...
impl<'a, T, N, V> SendableSuccessorIterator<'a, T, N, V>
where
    T: Numeric + Display,
    N: BfsNode<T, V, Arc<HashableSignatureVariables>>,
    V: TransitionInterface + Clone,
{
    ...
    pub fn new(node: Arc<N>, generator: &'a SuccessorGenerator<V, Arc<V>, Arc<Model>>) -> Self {
        Self {
            node,
            generator,
...
impl<'a, T, N, V> Iterator for SendableSuccessorIterator<'a, T, N, V>
where
    T: Numeric + Display,
    N: BfsNode<T, V, Arc<HashableSignatureVariables>>,
    V: TransitionInterface + Clone,
{
    type Item = (Arc<N>, Arc<V>);
