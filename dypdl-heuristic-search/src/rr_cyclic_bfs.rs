...
/// Performs round-robin cyclic best-first search.
///
    ...
    F: Fn(T, T, &StateInRegistry, &dypdl::Model) -> T,
{
    ...
    loop {
        let current_depth = if global_mode {
            let mut f_h_min = None;
            let mut arg_min = None;
            for (j, list) in open.iter_mut().enumerate() {
                while list.peek().map_or(false, |peek| *peek.0.closed.borrow()) {
                    list.pop();
                }
                let f_h = list
                    .peek()
                    .map(|peek| (peek.0.f.borrow().unwrap(), peek.0.h.borrow().unwrap()));
                if f_h.is_some() && (f_h_min.is_none() || f_h.unwrap() < f_h_min.unwrap()) {
                    f_h_min = f_h;
                    arg_min = Some(j);
                }
            }
        ...
        if let Some(Reverse(node)) = open[current_depth].pop() {
                        ...
                        if let Some((successor, dominated)) = registry.insert(state, g, constructor)
                        {
                            ...
                            if let Some(h) = h {
                                ...
                                *successor.f.borrow_mut() = Some(f);
                                ...
                                open[current_depth + 1].push(Reverse(successor));
                                generated += 1;
                            }
                        }
                    }
