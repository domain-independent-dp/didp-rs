...
use dypdl_heuristic_search::{create_labeling, FEvaluatorType, Parameters, Search};
use pyo3::prelude::*;
use std::rc::Rc;

/// Labeling algorithm, a resource-constrained shortest path search.
///
/// This performs a best-first search where nodes are ordered by comparing resource variables
/// lexicographically before falling back to the dual bound (or cost, if the model has no dual
/// bound).
///
...
/// >>> print(solution.cost)
/// 1
#[pyclass(unsendable, name = "Labeling")]
pub struct LabelingPy(WrappedSolver<Box<dyn Search<Integer>>, Box<dyn Search<OrderedContinuous>>>);
...
impl LabelingPy {
    ...
    fn new(
        ...
        if model.float_cost() {
            ...
            let solver = create_labeling(
                Rc::new(model.inner_as_ref().clone()),
                parameters,
                f_evaluator_type,
            );
            Ok(LabelingPy(WrappedSolver::Float(solver)))
        } else {
            let primal_bound = if let Some(primal_bound) = primal_bound {
            ...
            Ok(LabelingPy(WrappedSolver::Int(solver)))
        }
    }
